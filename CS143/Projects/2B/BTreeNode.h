...
// The max number of records that can be stored in a node is 84.
// 1024-sizeof(int)-sizeof(int)-sizeof(Record) = floor(1016/sizeof(Record)) = 84.
static const int MAX_RECORDS = 84; 
static const int MAX_NODE_SIZE = 1016;
static const int LEAF_SIZE = sizeof(int) + sizeof(RecordId); 
...
class BTLeafNode {
  ...
  private: 
    ...
    */
	...
	// Writes entry to buffer.
	RC buffInsert(const int key, const RecordId rid, const int eid);
	RC del(const int eid);
	RC shift(const int eid);
	bool isFull();
	void keyIncrement(bool increment);
	
}; 



static const int MAX_RECORDS_NONLEAF = 127;
/**
...
class BTNonLeafNode {
  ...
  private:
    ...
    */
    char buffer[PageFile::PAGE_SIZE];
	RC del(const int pos);
	RC shift(const inc pos);
	bool isFull();
	void keyIncrement(bool inc);
}; 
