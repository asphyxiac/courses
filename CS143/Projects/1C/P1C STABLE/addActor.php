...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	} else if (check_name($values['actFirstName']) == false ) {
    	$errors['actFirstName'] = "Invalid first name.";
	}
    ...
    if (check_input($values['actDOD'])) {
		if (verifyDate($values['actDOD']) == false)
        $errors['actDOD'] = "Invalid date format.";
    }
	
    if (check_input($values['actLastName'])) {
		if (check_name($values['actLastName']) == false )
